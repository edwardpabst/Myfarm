...
describe FarmsController do
  ...
  def mock_farm(stubs={})
    (@mock_farm ||= mock_model(Farm).as_null_object).tap do |farm|
      farm.stub(stubs) unless stubs.empty?
    end
  end

  describe "GET index" do
    it "assigns all farms as @farms" do
      Farm.stub(:all) { [mock_farm] }
      get :index
      assigns(:farms).should eq([mock_farm])
    end
  end

  describe "GET show" do
    it "assigns the requested farm as @farm" do
      Farm.stub(:find).with("37") { mock_farm }
      get :show, :id => "37"
      assigns(:farm).should be(mock_farm)
    end
  end

  describe "GET new" do
    it "assigns a new farm as @farm" do
      Farm.stub(:new) { mock_farm }
      get :new
      assigns(:farm).should be(mock_farm)
    end
  end
  ...
  describe "POST create" do
    ...
    describe "with valid params" do
      it "assigns a newly created farm as @farm" do
        Farm.stub(:new).with({'these' => 'params'}) { mock_farm(:save => true) }
        post :create, :farm => {'these' => 'params'}
        assigns(:farm).should be(mock_farm)
      end

      it "redirects to the created farm" do
        Farm.stub(:new) { mock_farm(:save => true) }
        post :create, :farm => {}
    ...
    describe "with invalid params" do
      it "assigns a newly created but unsaved farm as @farm" do
        Farm.stub(:new).with({'these' => 'params'}) { mock_farm(:save => false) }
        post :create, :farm => {'these' => 'params'}
        assigns(:farm).should be(mock_farm)
      end

      it "re-renders the 'new' template" do
        Farm.stub(:new) { mock_farm(:save => false) }
        post :create, :farm => {}
  ...
  describe "PUT update" do
    ...
    describe "with invalid params" do
      it "assigns the farm as @farm" do
        Farm.stub(:find) { mock_farm(:update_attributes => false) }
        put :update, :id => "1"
        assigns(:farm).should be(mock_farm)
      end
