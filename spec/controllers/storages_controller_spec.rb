...
describe StoragesController do
  ...
  def mock_storage(stubs={})
    (@mock_storage ||= mock_model(Storage).as_null_object).tap do |storage|
      storage.stub(stubs) unless stubs.empty?
    end
  end

  describe "GET index" do
    it "assigns all storages as @storages" do
      Storage.stub(:all) { [mock_storage] }
      get :index
      assigns(:storages).should eq([mock_storage])
    end
  end

  describe "GET show" do
    it "assigns the requested storage as @storage" do
      Storage.stub(:find).with("37") { mock_storage }
      get :show, :id => "37"
      assigns(:storage).should be(mock_storage)
    end
  end

  describe "GET new" do
    it "assigns a new storage as @storage" do
      Storage.stub(:new) { mock_storage }
      get :new
      assigns(:storage).should be(mock_storage)
    end
  end
  ...
  describe "POST create" do
    ...
    describe "with valid params" do
      it "assigns a newly created storage as @storage" do
        Storage.stub(:new).with({'these' => 'params'}) { mock_storage(:save => true) }
        post :create, :storage => {'these' => 'params'}
        assigns(:storage).should be(mock_storage)
      end

      it "redirects to the created storage" do
        Storage.stub(:new) { mock_storage(:save => true) }
        post :create, :storage => {}
    ...
    describe "with invalid params" do
      it "assigns a newly created but unsaved storage as @storage" do
        Storage.stub(:new).with({'these' => 'params'}) { mock_storage(:save => false) }
        post :create, :storage => {'these' => 'params'}
        assigns(:storage).should be(mock_storage)
      end

      it "re-renders the 'new' template" do
        Storage.stub(:new) { mock_storage(:save => false) }
        post :create, :storage => {}
  ...
  describe "PUT update" do
    ...
    describe "with valid params" do
      it "updates the requested storage" do
        Storage.should_receive(:find).with("37") { mock_storage }
        mock_storage.should_receive(:update_attributes).with({'these' => 'params'})
        put :update, :id => "37", :storage => {'these' => 'params'}
      end

      it "assigns the requested storage as @storage" do
        Storage.stub(:find) { mock_storage(:update_attributes => true) }
        put :update, :id => "1"
        assigns(:storage).should be(mock_storage)
      end
    ...
    describe "with invalid params" do
      it "assigns the storage as @storage" do
        Storage.stub(:find) { mock_storage(:update_attributes => false) }
        put :update, :id => "1"
        assigns(:storage).should be(mock_storage)
      end
  ...
  describe "DELETE destroy" do
    ...
    it "redirects to the storages list" do
      Storage.stub(:find) { mock_storage }
      delete :destroy, :id => "1"
